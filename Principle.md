...
<center>
$$
\Sigma_z = \sqrt{\beta_1\mathbf{y}+\beta_2^2} \tag{2}
$$
</center>
...
+ Apply the conventional SIM algorithm to reconstruct two corrupted SR image
+ Categorize these two SR images as the input and target data of the training dataset, respectively.
+ Repeat the recorruption operations 3~5 times for each raw image stack with different parameters to enrich the dataset.
...
<center>
$$
\mathbf{y}=\beta_1\mathcal{P}(\mathbf{x})+\mathcal{N}(0,\beta_2^2) \tag{3}
$$
</center>
For a Poisson function $\mathcal{P}(\mathbf{\lambda})$, 
it can be approximated by a Gaussian function as $\mathcal{N}(\lambda,\lambda)$ with low error when the value of $\lambda$ is not too low.
...
<center> 
$$
min\qquad ||\phi(\mathbf{y})-\mathbf{x}||_2^2 \tag{5}
$$
</center>
...
In Eq. 12, we have $E(\sum_{i=j}c_{ij}n_{A,i}\cdot n_{B,j})=0$ due to the property of recorruption operation,
and $E(\sum_{i\ne j}c_{ij}n_{A,i}\cdot n_{B,j})=0$ since the noise from different raw images is independent. 
Therefore, the recorrupted SR image $\mathbf{Y_A}$ and $\mathbf{Y_B}$ still satisfies the independent criteria,
 and the effectiveness of RES-SIM is proved.
