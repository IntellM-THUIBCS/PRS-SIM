...
parser.add_argument('--smpl_name', type=str, default='Lyso_test', help='the name of the sample')
parser.add_argument('--net_type', type=str, default='unet', help='network type') 


parser.add_argument('--model_dir', type=str, default='./pretrained_models/Lyso_unet_0/models', help='the folder saveing the model file')
parser.add_argument('--model_name', type=str, default='100000_G', help='the name of the model file (.pth)')

parser.add_argument('--test_patch_size', type=int, default=1024, help='the crop size of the image')
parser.add_argument('--model_z_size', type=int, default=8, help='the z size of each tile')
...
opt_name = 'train_'+ params.net_type + '_3D' +'.json'
...
for test_data in test_loader:    
    ...
    E_img = np.zeros((params.test_z_size,params.test_patch_size,params.test_patch_size), dtype=np.float32)
    R_img = np.zeros((params.test_z_size,params.test_patch_size,params.test_patch_size), dtype=np.float32)
    ...
    tifffile.imsave(save_img_path_tif, np.uint16(np.max(E_img,axis=0)))
    ...
    tifffile.imsave(save_img_path_tif, np.uint16(np.max(R_img,axis=0)))
