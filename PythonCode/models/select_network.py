...
def define_G(opt):
    ...
    net_para1 = None
    net_para2 = None
    if opt_net['net_para1'] is not None:
    ...
    elif net_type == 'rcan':  # Rcan
        from models.network_rcan import RCAN as net
        netG = net(para1=net_para1,para2=net_para2) if (net_para1 is not None) and (net_para2 is not None) else net()
        
    elif net_type == 'rdn':  # RDN
        from models.network_rdn import RDN as net
        netG = net(para1=net_para1) if (net_para1 is not None) else net()
        
    elif net_type == 'unet3D':  # 3D U-net
        from models.network_unet_3D import UNet as net
        netG = net(in_nc=opt_net['in_nc'],
                   out_nc=opt_net['out_nc'])
