...
# convert 2/3/4-dimensional torch tensor to uint16
def tensor2uint16(img, norm_flag=False):
    if norm_flag:
...
def tensor2uint16_norm(img, norm_flag=False):
    img_ = img.cpu().numpy().astype(np.float32)
    
    img_ = (img_-np.min(img_))/(np.max(img_)-np.min(img_)) * 65535.

    return np.uint16(img_)

def tensor2float(img, norm_flag=False):
    img_ = img.cpu().numpy().astype(np.float32)
    if norm_flag:
...
def calculate_psnr_peak(img1, img2, border=0):
...
def calculate_psnr_peak3D(img1, img2, border=0):
    # img1 is the ground-truth image
    if not img1.shape == img2.shape:
        raise ValueError('Input images must have the same dimensions.')
    Z = img1.shape[0]
    val = 0
    for z in range(Z):
        val += calculate_psnr_peak(img1[z,...],img2[z,...],border)
...
# SSIM
# --------------------------------------------
def calculate_ssim_peak(img1, img2, border=0):
    '''calculate SSIM
    ...
    if not img1.shape == img2.shape:
        raise ValueError('Input images must have the same dimensions.')
    if not img1.ndim == 2:
        raise ValueError('Input images must have the two dimensions.')
    h, w = img1.shape[:2]
    img1 = img1[border:h-border, border:w-border]
    img2 = img2[border:h-border, border:w-border]
    peak = np.max(img1)
...
def calculate_ssim_peak3D(img1, img2, border=0):
    if not img1.shape == img2.shape:
        raise ValueError('Input images must have the same dimensions.')
    Z = img1.shape[0]
    val = 0
    for z in range(Z):
        val += calculate_ssim_peak(img1[z,...],img2[z,...],border)
